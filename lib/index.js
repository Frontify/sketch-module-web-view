...
function WebUI (context, frameLocation, options) {
  ...
  panel.setStyleMask(options.styleMask || (
    options.resizable
    ? (NSTexturedBackgroundWindowMask | NSWindowStyleMaskTitled | NSResizableWindowMask | NSClosableWindowMask | NSFullSizeContentViewWindowMask)
    : (NSWindowStyleMaskTitled | NSUtilityWindowMask | NSClosableWindowMask | NSFullSizeContentViewWindowMask | NSBorderlessWindowMask)
  ))
  ...
  if (options.toolbar) {
    var windowLoc = context.document.documentWindow().frame()
    var toolbarLoc = options.toolbar.webView.frame()

    panel.setFrameTopLeftPoint(NSMakePoint(
      windowLoc.origin.x + toolbarLoc.origin.x - panel.frame().size.width - 1, // 1 pixel border
      windowLoc.origin.y + toolbarLoc.size.height - options.toolbarLocation * 40 - (options.toolbarLocation > 0 ? 5 : 0))
    )
  } else {
    panel.center()
  }

  if (!options.nochildwindow) {
    panel.makeKeyAndOrderFront(null)
    var documentWindow = context.document.documentWindow()

    documentWindow.addChildWindow_ordered(panel, NSWindowAbove)
  }

  if (options.closeOnResize) {
    var windowDelegate = new MochaJSDelegate({
      'windowDidResize:': function (notification) {
        var windowLoc = context.document.documentWindow().frame()
        var toolbarLoc = options.toolbar.webView.frame()
